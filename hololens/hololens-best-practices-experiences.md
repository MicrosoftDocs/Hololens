...
Our customers and partners have created amazing experiences with HoloLens, and we see multiple fantastic applications and scenarios created in museums, aquariums, and attractions across the world. Our partners have multiple methods of deploying and maintaining these, and this guide focusses on the best practices for your application, your technical architecture, and your device management and provisioning.

By following the guidance below, you will be able to ensure your experience is scalable, easy to manage, and ready for use with your experience partners, to give your customers a delightful time with your experiences.

## Areas of focus

During this guide, we will focus on several key areas which will work together to form an overall solution.  

1. Application Considerations
1. Environment Considerations
1. Choosing the right Device Management options
1. Policy configuration
1. “Out of Box” production line
1. Troubleshooting and device reset
...
Your experience will run within the context of a UWP application which will run on the HoloLens device. Development support is outside the context of this document, however there are a few elements that will be useful to consider during your development.  

### Single Self-Contained UWP Application

When deploying the application following our recommended methodology, we will deploy a “Single App Kiosk” which will automatically launch your application. Users will not have access to the HoloLens Operating System (OS) or Shell, which means that all activities that are used by your experience should be contained within this one application.  
...
In any IT environment, there may be intermittent issues with networking, or a need to reset a malfunctioning application. In a single app kiosk, there is no way to kick out to a “Settings” screen to perform this minor troubleshooting.  
...
HoloLens seamlessly blends holographic assets into real world surroundings appending stable and accurate holograms by tracking users in a space. Proper tracking is at the centre of how HoloLens places experiences in your environment. To maintain proper tracking performance and for optimum usage, there are some Environment considerations that need to be adhered to for HoloLens to operate as intended. Please see HoloLens environment considerations.
...
|     Proximity of the user to   items in the space                                                                                                  |     HoloLens cameras can see no closer than 15cm from   an object. See [Proximity of the user to items in the space](hololens-environment-considerations.md#proximity-of-the-user-to-items-in-the-space)                                                                                                                                                                                                                       |
...
|     Battery Information                                                                                                                            |     2 - 3 hours battery usage time. USB-C battery packs   can be utilised to extend this time.                                                                                                                                                                                                                                           |

For experiences that are likely to last more than 5 to 10 minutes, it is recommended to launch eye calibration at the start of your experience.
...
The Auto Logon to a visitor profile allows you jump straight into the action without ever seeing the login screen. The device will be launched straight into the kiosk experience using the visitor profile. This is a required setting and can be managed using the custom policy OMA-URI below …
| Setting      | Value                                                                   |
|--------------|-------------------------------------------------------------------------|
...
The Tenant Lockdown CSP keeps devices on the organization's tenant by locking them to that tenant even through device reset or reflash. It enables HoloLens 2 to be tied to MDM enrolment using Autopilot only and can be managed using the custom policy OMA-URI below …
| Setting      | Value                                                    |
|--------------|----------------------------------------------------------|
...
It is advised that all devices are updated to the latest build using [Advanced Recovery Companion](https://www.microsoft.com/store/productId/9P74Z35SFRS8) (ARC) which can be downloaded from Microsoft Store.
...
For updates with a provisioning package, it is recommended to reflash the device using Advanced Recovery Companion (ARC) and apply a new provisioning package with your specific configurations for any updates.
...
2. The device will automatically start the Out of Box Experience, which should not be interacted with once the internet connection is established. The device may restart during the OOBE but allow the process to finish before interacting with the device.
...
For busy working environments there are some best practices to follow to ensure the HoloLens device is always ready and prepared for use. Here is a list of cleaning and charging information to ensure optimal use.
...
Additionally, it is important to report any issues, through the Feedback Hub app available on the HoloLens device. [Give us feedback](hololens-feedback.md)
