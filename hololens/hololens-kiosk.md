...
| Only specific Azure AD user is used to automatically sign into HoloLens and experience kiosk targeted for that Azure AD user. | [Multiple app assigned access profile for one Azure AD account for one app](hololens-kiosk-reference.md#multiple-app-assigned-access-profile-for-one-azure-ad-account-for-one-app) <br> Specify that user's email address in [MixedReality/AutoLogonUser](/windows/client-management/mdm/policy-csp-mixedreality#mixedreality-autologonuser) policy. | • [Microsoft Intune custom template](/hololens/hololens-kiosk?tabs=intunecustom%2Cautologon#steps-in-configuring-kiosk-mode-for-hololens) <br> • [Runtime provisioning - Multi app](hololens-kiosk.md?tabs=ppkgmak#steps-in-configuring-kiosk-mode-for-hololens) | You may choose to have only one AAD user sign-in. Once they user has signed in once the device will continue to sign them in automatically and never sign-out. |
