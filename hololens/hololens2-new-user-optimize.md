...
- **FIDO2 Security key** : A security key will allow someone to sign in with a PIN (for the key) and a touch, this is fast. Also when setting up a new user on a device using a FIDO2 security key won't require them to using their phone for Multi-factor authentication.
