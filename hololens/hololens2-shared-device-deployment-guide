...
2. Navigate to [New Group - Azure Active Directory admin center](https://aad.portal.azure.com/#blade/Microsoft_AAD_IAM/GroupsManagementMenuBlade/AllGroups) blade and create a Azure AD **Security** Group to manage the HoloLens 2 shared user accounts. See [Create a basic group and add members - Azure Active Directory](https://docs.microsoft.com/en-us/azure/active-directory/fundamentals/active-directory-groups-create-azure-portal#:~:text=To%20create%20a%20basic%20group%20and%20add%20members,page%2C%20select%20Groups%20and%20then%20select%20New%20group.) for step by step instructions.
...
#### Auto Login
You can also use AutoLogonUser policy to automatically on login to the device with an identity tied to that device. This bypasses the HoloLens 2 login experience, and the user will be able to pick up the device and start using the device straight away. 
...
1. Identity – Users cannot use IRIS to sign in on the HoloLens 2 device and are unable to access their work account related content in M365. 
...
5. MFA ownership – MFA should be owned by a central authority for shared accounts.
